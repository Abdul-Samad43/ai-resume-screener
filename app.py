...
st.markdown("""
    ...
    <p style='text-align: center; color: #6b7280;'>
        Resume aur Job Description compare karo — AI se!
    </p>
...
with col1:
    ...
    uploaded_file = st.file_uploader("PDF resume upload karo", type="pdf")

with col2:
    st.subheader("💼 Job Description")
    job_description = st.text_area(
        "Job description yahan paste karo",
        height=200,
...
if st.button("🔍 Resume Analyze Karo", use_container_width=True):
    if not uploaded_file:
        st.error("❌ Resume upload karo pehle!")
    elif not job_description:
        st.error("❌ Job description likho pehle!")
    else:
        # Yeh pehle define karo ← spinner se bahar
        analysis = None
        ai_raw = ""
